...
**运行视频处理主脚本：**
```bash
uv run python src/transvideo.py {PATH_TO_VIDEO}
```
*可选参数：*
- `--stages {audio,transcribe,translate,srt,compile}`: 运行指定的阶段。
- `--soft`: 以软字幕的形式将字幕外挂在视频上（可以在播放器中关闭），而不是直接烧录进视频。

**运行 SRT 转录文本脚本：**
```bash
uv run python src/srt_to_transcript.py input.srt [-o output.txt]
```
...
- **工具使用**: 优先使用 `uv` 进行环境管理和脚本运行，而不是全局的 `pip`（尽管 README 中仍有 `pip install` 的历史遗留描述）。适用时请使用 `Makefile` 中的命令（如 `make build`, `make run`, `make publish`）。
- **源码结构**: 所有的源代码和测试文件统一存放在 `src/` 目录下。
- **容错与恢复**: 主脚本 (`transvideo.py`) 会保留中间文件，以便在流程失败时能够从特定阶段恢复。在修改数据处理流水线时，请确保维持这一工作流特性。
